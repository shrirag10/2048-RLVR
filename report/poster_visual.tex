...
% ─── Semi-gradient SARSA ───────────────────────────────────────────
\begin{sectionbox}[{\raisebox{-1pt}{\tikz\fill[success] (0,0) rectangle (0.35,0.35);}\; \agentname{Semi-gradient SARSA} $|$ Only 120 Parameters!}]{success}

\begin{calloutbox}[success]
{\bfseries\small Sutton \& Barto Algorithm 10.1: 120 params beats 330K--494M deep nets!}
\end{calloutbox}

\vspace{2pt}
{\small
$\hat{q}(s,a,\mathbf{w})=\mathbf{w}_a^\top\phi(s)$, \quad $\mathbf{w}\in\mathbb{R}^{4\times 30}$

\textbf{30-dim features (\S9.5):} log-tile values, empty ratio, merge potential, 4 monotonicity scores, corner bonus, smoothness, snake pattern, edge sum.

Semi-gradient SARSA (\S10.1): $\mathbf{w}_a \leftarrow \mathbf{w}_a + \alpha\delta\phi(s)$, \quad $\delta = R + \gamma\hat{q}(S',A') - \hat{q}(S,A)$
}
\end{sectionbox}
...
Semi-grad.\ SARSA & 2,456 & 11,436 & 1024 & 6.0\% & $<$1\% & \textcolor{success}{\textbf{120}} \\
...
SAC & 1,255 & 7,882 & 512 & $<$1\% &: & 330K \\
QR-DQN & 995 & 5,176 & 512 & $<$1\% &: & 430K \\
\rowcolor{dangerlight}
PPO & \textcolor{danger}{962} & 4,900 & 512 & $<$1\% &: & 330K \\
GRPO 0.5B & 704 & 704 & 64 &: &: & 494M \\
\bottomrule
\multicolumn{7}{@{}l}{\footnotesize GRPO: 0.5B = Qwen2.5-0.5B (3-stage); 1.5B = Qwen2.5-1.5B (3-stage). Semi-gradient SARSA = S\&B Alg.\ 10.1.}
\end{tabular}
}
\end{center}
\end{sectionbox}
...
\node[font=\tiny\bfseries, anchor=north, text=success] at (1.9, -0.15) {SARSA};
...
\fill[danger!70] (5.7,0) rectangle (6.05, 0.471); \draw[danger] (5.7,0) rectangle (6.05, 0.471);
\node[font=\tiny\bfseries, anchor=north, text=danger] at (5.7, -0.15) {SAC};
\node[font=\tiny, anchor=south, text=danger] at (5.875, 0.52) {1255};
% QR-DQN
...
\foreach \i/\tile/\agent/\col in {
    0/2048/DQN/accent, 1/1024/SARSA/success, 2/512/A2C/info,
    3/512/PPO/info, 4/512/QR-DQN/purple, 5/256/SAC/danger}{
...
{\tiny\bfseries\color{success} SARSA $|$ 2,456}
\end{minipage}
\hfill
\begin{minipage}[t]{0.16\linewidth}
\centering
...
% Semi-gradient SARSA bar
\fill[success!40, rounded corners=2pt] (8.9, 0) rectangle (9.7, 0.48);
\node[font=\footnotesize\bfseries] at (9.3, 0.62) {SARSA};
...
{\small
DQN's \textbf{experience replay} (100K buffer, S\&B \S8.2) revisits rare high-scoring trajectories thousands of times. On-policy methods (\S5.4) discard each trajectory after one update, catastrophic for sparse rewards.
}
\end{sectionbox}

% ─── Key Finding 2 ─────────────────────────────────────────────────
\begin{sectionbox}[{\Large\color{white} 2}\; Feature Engineering vs.\ Deep Learning (S\&B \S9.5)]{success}
...
\node[font=\normalsize, text=danger] at (4.8, -0.7) {PPO: 962 \quad QR: 995 \quad SAC: 1,255};
...
{\small
Hand-crafted monotonicity \& corner features (Sutton \& Barto \S9.5) provide stronger inductive bias than CNNs learning from scratch. \textbf{2750$\times$ parameter efficiency!} But Semi-gradient SARSA plateaus at ${\sim}2500$ (linear capacity ceiling, \S9.4).
}
\end{sectionbox}
...
\textcolor{info}{$\uparrow$ Modest} & +56--79\% & A2C, SAC \\
\textcolor{gray}{$\rightarrow$ Plateau} & +7\% & Semi-grad.\ SARSA \\
...
\node[font=\footnotesize\bfseries, anchor=west, text=success] at (0.1,0.2) {SARSA};
...
\textcolor{success}{\textbf{2.}} Semi-gradient SARSA (120 params, S\&B Alg.\ 10.1) beats all deep on-policy agents \quad

...
\begin{keybox}
\begin{center}
{\large\bfseries 7 Agents} benchmarked end-to-end \quad\textbullet\quad
...
\node[box] (llm) at (2.6,0) {Qwen 0.5B};
...
{\small
\textbf{Model:} Qwen2.5-0.5B, QLoRA 4-bit, 5.2M trainable (1.05\%)
...
{\footnotesize\bfseries Staged scheduling required!} All rewards at once $\to$ reward hacking (model maximizes length, ignores format).
\end{calloutbox}
}
\end{sectionbox}
...
GRPO (LLM) & 1,816 & 1,816 & 128 & --- & --- & 494M \\
SAC$^*$ & 1,131 & 5,566 & 512 & $<$1\% & --- & 330K \\
QR-DQN & 995 & 5,176 & 512 & $<$1\% & --- & 430K \\
\rowcolor{dangerlight}
PPO & \textcolor{danger}{962} & 4,900 & 512 & $<$1\% & --- & 330K \\
\bottomrule
\multicolumn{7}{@{}l}{\footnotesize $^*$SAC trained $\sim$4M steps (in progress); all others completed 5M.}
\end{tabular}
}
\end{center}
\end{sectionbox}
...
% ─── Key Finding 5 ─────────────────────────────────────────────────
\begin{sectionbox}[{\Large\color{white} 5}\; GRPO: Format Before Strategy]{purple}
...
\node[stage=purple!25] (s2) at (3.3,0) {Stage 2: Direction};
\node[stage=purple!35] (s3) at (6.6,0) {Stage 3: Game};

\draw[arr] (s1) -- (s2);
\draw[arr] (s2) -- (s3);

\node[font=\footnotesize, text=success, below=3pt of s1] {\checkmark\,Done};
\node[font=\footnotesize, text=gray, below=3pt of s2] {Next};
\node[font=\footnotesize, text=gray, below=3pt of s3] {Future};
\end{tikzpicture}
}
\end{center}

{\small
Simultaneous rewards $\to$ \textcolor{danger}{\textbf{reward hacking}} (model generates filler text). Must learn XML format first, then strategy.
}
\end{sectionbox}
...
\begin{itemize}[nosep, leftmargin=10pt, itemsep=2pt]
    \item[\textcolor{accent}{$\triangleright$}] Complete SAC 5M training
    \item[\textcolor{accent}{$\triangleright$}] Staged GRPO (Stage 2: +game reward)
...
\textcolor{purple}{\textbf{4.}} GRPO needs staged rewards \quad
